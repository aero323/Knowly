...
本版本重点优化 Electron 桌面字幕浮层的多语言现场展示，让大会投屏和多语同传场景更稳、更清爽。

### Desktop App

- 字幕浮层多目标语言改为独立分列展示，每种目标语言一列，列内独立滚动自己的字幕流。
- 多语言浮层按语言数横向扩展窗口宽度，滚动模式高度只跟可见句数相关，避免语言越多窗口越高。
- 新增桌面字幕语言状态桥接，浮层可保留单个语言列的连接异常 / 重连状态，其他语言继续显示和滚动。
- 滚动模式下各语言列按同一 source sequence 对齐；缺失译文时该列显示占位，不阻塞其他语言。
- 浮层视觉进一步精简，只保留顶部语种胶囊，移除“句 X”“在线”“历史”“最新”“最后句”等冗余信息。
- 共享原文区域去掉“原文”小标签，减少投屏时的视觉干扰。
- 全屏字幕针对会议投屏优化：译文和原文字号放大，字幕整体上移，并在底部留出更大的安全空白，降低被会场人头遮挡的概率。
